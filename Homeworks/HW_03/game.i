...
void updatePlayer() {
    if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<5))) && (player.col - player.cdel) >= 0) {
        player.col -= player.cdel;
    }
    if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<4))) && (player.col + player.cdel + player.width) <= 240) {
        player.col += player.cdel;
    }
    if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<6))) && (player.row - player.rdel) >= 0) {
        player.row -= player.rdel;
    }
    if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<7))) && (player.row + player.rdel + player.height) <= 160) {
        player.row += player.rdel;
    }
}
void drawPlayer() {
...
void initBombs() {
    for (int i = 0; i < 50; i++) {
        ...
        bombs[i].height = 10;
        bombs[i].width = 10;
        bombs[i].color = ((rand()%255) | (rand()%255)<<5 | (rand()%255)<<10);
        bombs[i].active = 0;
    }
}
...
void drawBomb(BOMB* bomb) {
    if (bomb->active) {
        drawRect(bomb->col, bomb->oldRow,bomb->width, bomb->height, 0);
