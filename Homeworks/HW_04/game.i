...
# 1 "tiger.h" 1
# 21 "tiger.h"
extern const unsigned short tigerBitmap[288];
...
void initGame() {
    ...
    unsigned short colors[0] = {((31) | (31)<<5 | (31)<<10)};
    DMANow(3, tigerPal, ((unsigned short *)0x5000000), 256);
    ((unsigned short *)0x5000000)[256-1] = colors[0];
...
void drawGame() {
    fillScreen4(((unsigned short *)0x5000000)[0]);
...
void updateTiger() {
    ...
    if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<5))) && (tiger.col - tiger.cdel) > 0 ) {
        tiger.col -= tiger.cdel;
    }
    if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<4))) && (tiger.col + tiger.cdel + tiger.width) < 240) {
        tiger.col += tiger.cdel;
    }
    if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<6))) && (tiger.row - tiger.rdel) > 0) {
        tiger.row -= tiger.rdel;
    }
    if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<7))) && (tiger.row + tiger.rdel + tiger.height) < 160) {
        tiger.row += tiger.rdel;
    }
}
...
void initBunnies() {
    for (int i = 0; i < 4; i++) {
        bunnies[i].row = rand() % (160 - 10);
        bunnies[i].col = rand() % (240 - 10);
        bunnies[i].cdel = 2;
        bunnies[i].rdel = 2;
        bunnies[i].height = 10;
        bunnies[i].width = 10;
...
void updateBunny(BUNNY *bunny) {
    ...
    if (!bunny->erased) {
        ...
        if ((bunny->row + bunny->height - 1) >= 160 - 2) {
            bunny->rdel *= -1;
        }
        ...
        if ((bunny->col + bunny->width - 1) >= 240 - 2) {
            bunny->cdel *= -1;
        }
        ...
        bunny->respawn += 1;
        if (bunny->respawn >= 100) {
            bunny->respawn = 0;
            bunny->erased = 0;
            bunny->col = rand() % (240 - 10);
            bunny->row = rand() % (160 - 10);
        }
    }
}
